...
class CalendarSync:
    ...
    def _convert_ical_to_google_event(self, event):
        ...
        if attendee:
            # Find if the current user declined the event
            user_email = self.calendar_name.lower().replace(' ', '.') + '@gmail.com'  # Rough approximation
            for att in attendee if isinstance(attendee, list) else [attendee]:
                att_str = str(att)
                if user_email in att_str.lower() and 'PARTSTAT=DECLINED' in att_str.upper():
                    event_status = 'cancelled'
                    logger.info(f"Event declined by current user in source calendar: {event.get('SUMMARY')}")
                    break
        ...
        if rrule:
            ...
            if recurrence_id:
                ...
                # Save the RECURRENCE-ID information for later use
                if 'extendedProperties' not in google_event:
                    google_event['extendedProperties'] = {'private': {}}
                google_event['extendedProperties']['private']['recurrenceException'] = 'true'
                google_event['extendedProperties']['private']['recurrenceExceptionDate'] = exception_date_str
